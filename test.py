    ...
    if input != None:
        ...
        for row in df_selection.iterrows():
            ...
            fig.add_scattermapbox(
                ...
                marker=dict(size=13),
                text=upcoming_stops["stop_name"] + " delay: " + str(upcoming_stops["arrival_delay"]),
            )
        fig.update_mapboxes(
        center={"lat": df_selection["lat"].mean(), "lon": df_selection["lon"].mean()},
        zoom=11,
        )
        ...
        lat=df_combine["lat"],
        lon=df_combine["lon"],
        text=df_combine["route_long_name"] + " to: " + df_combine["stop_name"],
    )
        fig.update_mapboxes(
